(function(root){
  ...
  function renderInteriorDelta(){
    ...
    const deltas=enriched.map((item)=>{const {key,priority,label,unit,before,after,tone,strength,group,groupLabel}=item,dominant=groupDominant.get(group)===item,state=groupState(group),globalCriticalCause=globalCritical===item,secondaryCriticalCause=dominant&&state==='worsening'&&!globalCriticalCause,consequenceTier=globalCriticalCause?'primary':secondaryCriticalCause?'secondary':'context',consequenceTierLabel=globalCriticalCause?'Hauptfolge':secondaryCriticalCause?'Sekundärfolge':'Folge',consequence=(consequenceLabel[group]||consequenceLabel.other)[state];return`<span class="coach-delta" data-metric="${esc(key)}" data-priority="${priority}" data-effect-group="${group}" data-group-dominant="${dominant}" data-group-state="${state}" data-global-critical="${globalCriticalCause}" data-secondary-critical="${secondaryCriticalCause}" data-consequence-tier="${consequenceTier}" data-consequence-label="${esc(consequence)}" data-impact="${tone}" data-magnitude="${strength}" aria-label="${esc(`${groupLabel}, ${label}: ${format(before,unit)} zu ${format(after,unit)}, ${impactLabel[tone]}, ${magnitudeLabel[strength]}${dominant?', dominierende Gruppenwirkung':''}${globalCriticalCause?', gruppenübergreifend kritischste Ursache':''}${secondaryCriticalCause?', sekundäre kritische Ursache':''}; ${groupStateLabel[state]}; Konsequenz: ${consequence}; Einordnung: ${consequenceTierLabel}`)}"><b>${esc(label)}</b> <span data-before>${esc(format(before,unit))}</span><i aria-hidden="true">→</i><span data-after>${esc(format(after,unit))}</span></span>`;}).join(' · ');
