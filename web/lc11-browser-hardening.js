(function(root){
  ...
  function renderInteriorDelta(){
    ...
    const groupDominant=new Map();
    enriched.forEach((item)=>{const current=groupDominant.get(item.group);if(!current||item.groupRank<current.groupRank||(item.groupRank===current.groupRank&&item.priority<current.priority))groupDominant.set(item.group,item);});
    const deltas=enriched.map((item)=>{const {key,priority,label,unit,before,after,tone,strength,group,groupLabel}=item,dominant=groupDominant.get(group)===item;return`<span class="coach-delta" data-metric="${esc(key)}" data-priority="${priority}" data-effect-group="${group}" data-group-dominant="${dominant}" data-impact="${tone}" data-magnitude="${strength}" aria-label="${esc(`${groupLabel}, ${label}: ${format(before,unit)} zu ${format(after,unit)}, ${impactLabel[tone]}, ${magnitudeLabel[strength]}${dominant?', dominierende Gruppenwirkung':''}`)}"><b>${esc(label)}</b> <span data-before>${esc(format(before,unit))}</span><i aria-hidden="true">→</i><span data-after>${esc(format(after,unit))}</span></span>`;}).join(' · ');
