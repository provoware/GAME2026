(function(root){
  ...
  function renderInteriorDelta(){
    ...
    const deltas=enriched.map((item)=>{const {key,priority,label,unit,before,after,tone,strength,group,groupLabel}=item,dominant=groupDominant.get(group)===item,state=groupState(group),globalCriticalCause=globalCritical===item;return`<span class="coach-delta" data-metric="${esc(key)}" data-priority="${priority}" data-effect-group="${group}" data-group-dominant="${dominant}" data-group-state="${state}" data-global-critical="${globalCriticalCause}" data-impact="${tone}" data-magnitude="${strength}" aria-label="${esc(`${groupLabel}, ${label}: ${format(before,unit)} zu ${format(after,unit)}, ${impactLabel[tone]}, ${magnitudeLabel[strength]}${dominant?', dominierende Gruppenwirkung':''}${globalCriticalCause?', gruppenübergreifend kritischste Ursache':''}; ${groupStateLabel[state]}`)}"><b>${esc(label)}</b> <span data-before>${esc(format(before,unit))}</span><i aria-hidden="true">→</i><span data-after>${esc(format(after,unit))}</span></span>`;}).join(' · ');
