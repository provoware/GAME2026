(function(root){
  ...
  function renderInteriorDelta(){
    ...
    const consequenceLabel={security:{worsening:'Handlungsdruck steigt',improving:'Lage entspannt sich',mixed:'Lage bleibt volatil',stable:'Lage bleibt kontrollierbar'},resources:{worsening:'Spielraum sinkt',improving:'Puffer wächst',mixed:'Puffer bleibt unsicher',stable:'Puffer bleibt stabil'},agency:{worsening:'Optionen werden enger',improving:'Optionen wachsen',mixed:'Optionen bleiben widersprüchlich',stable:'Optionen bleiben stabil'},other:{worsening:'Folgewirkung verschärft sich',improving:'Folgewirkung verbessert sich',mixed:'Folgewirkung bleibt gemischt',stable:'Folgewirkung bleibt stabil'}};
    const globalCritical=enriched.filter((item)=>groupDominant.get(item.group)===item&&groupState(item.group)==='worsening').sort((a,b)=>a.groupRank-b.groupRank||a.priority-b.priority)[0]||null;
    const consequenceUrgency={security:['immediate','sofort',0],agency:['soon','bald',1],resources:['later','später',2],other:['later','später',3]};
    const actionAdvice={security:'Druck senken',agency:'Optionen sichern',resources:'Reserven sichern',other:'Folgewirkung prüfen'};
    const actionPriority={primary:['first','JETZT ZUERST',0],immediate:['next','DANACH',1],soon:['soon','ALS NÄCHSTES',2],later:['plan','EINPLANEN',3]};
    const secondaryCritical=enriched.filter((item)=>groupDominant.get(item.group)===item&&groupState(item.group)==='worsening'&&item!==globalCritical).sort((a,b)=>(consequenceUrgency[a.group]||consequenceUrgency.other)[2]-(consequenceUrgency[b.group]||consequenceUrgency.other)[2]||a.groupRank-b.groupRank||a.priority-b.priority),secondaryRank=new Map(secondaryCritical.map((item,index)=>[item,index+1]));
    const deltas=enriched.map((item)=>{const {key,priority,label,unit,before,after,tone,strength,group,groupLabel}=item,dominant=groupDominant.get(group)===item,state=groupState(group),globalCriticalCause=globalCritical===item,secondaryCriticalCause=dominant&&state==='worsening'&&!globalCriticalCause,secondaryCriticalRank=secondaryCriticalCause?(secondaryRank.get(item)||0):0,[consequenceUrgencyKey,consequenceUrgencyLabel]=consequenceUrgency[group]||consequenceUrgency.other,consequenceTier=globalCriticalCause?'primary':secondaryCriticalCause?'secondary':'context',consequenceTierLabel=globalCriticalCause?'Hauptfolge':secondaryCriticalCause?`Sekundärfolge Rang ${secondaryCriticalRank}`:'Folge',consequence=(consequenceLabel[group]||consequenceLabel.other)[state],actionKey=globalCriticalCause?'primary':secondaryCriticalCause?consequenceUrgencyKey:'later',[actionPriorityKey,actionPriorityLabel]=actionPriority[actionKey]||actionPriority.later,actionRecommendation=actionAdvice[group]||actionAdvice.other;return`<span class="coach-delta" data-metric="${esc(key)}" data-priority="${priority}" data-effect-group="${group}" data-group-dominant="${dominant}" data-group-state="${state}" data-global-critical="${globalCriticalCause}" data-secondary-critical="${secondaryCriticalCause}" data-secondary-rank="${secondaryCriticalRank}" data-consequence-urgency="${consequenceUrgencyKey}" data-consequence-urgency-label="${esc(consequenceUrgencyLabel)}" data-consequence-tier="${consequenceTier}" data-consequence-label="${esc(consequence)}" data-action-priority="${actionPriorityKey}" data-action-priority-label="${esc(actionPriorityLabel)}" data-action-recommendation="${esc(actionRecommendation)}" data-impact="${tone}" data-magnitude="${strength}" aria-label="${esc(`${groupLabel}, ${label}: ${format(before,unit)} zu ${format(after,unit)}, ${impactLabel[tone]}, ${magnitudeLabel[strength]}${dominant?', dominierende Gruppenwirkung':''}${globalCriticalCause?', gruppenübergreifend kritischste Ursache':''}${secondaryCriticalCause?`, sekundäre kritische Ursache Rang ${secondaryCriticalRank}, zeitliche Dringlichkeit ${consequenceUrgencyLabel}`:''}; ${groupStateLabel[state]}; Konsequenz: ${consequence}; Einordnung: ${consequenceTierLabel}${globalCriticalCause||secondaryCriticalCause?`; Handlung: ${actionPriorityLabel} – ${actionRecommendation}`:''}`)}"><b>${esc(label)}</b> <span data-before>${esc(format(before,unit))}</span><i aria-hidden="true">→</i><span data-after>${esc(format(after,unit))}</span></span>`;}).join(' · ');
