(function(root){
  ...
  function renderInteriorDelta(){
    ...
    const deltas=orderedKeys.map(({key,priority})=>{const [label,unit,polarity=1]=meta[key]||[key,'score',1],before=receipt.before[key],after=receipt.after[key],tone=impact(before,after,polarity),strength=magnitude(key,before,after);return`<span class="coach-delta" data-metric="${esc(key)}" data-priority="${priority}" data-impact="${tone}" data-magnitude="${strength}" aria-label="${esc(`${label}: ${format(before,unit)} zu ${format(after,unit)}, ${impactLabel[tone]}, ${magnitudeLabel[strength]}`)}"><b>${esc(label)}</b> <span data-before>${esc(format(before,unit))}</span><i aria-hidden="true">→</i><span data-after>${esc(format(after,unit))}</span></span>`;}).join(' · ');
