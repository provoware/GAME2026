(function(root){
  ...
  const pressureClass=(d)=>Math.max(d.rival||0,d.police||0,d.unrest||0)>=72?'pressure-critical':Math.max(d.rival||0,d.police||0,d.unrest||0)>=48?'pressure-watch':'pressure-calm';
  ...
  function renderMapInfo(selected,current,options,planned){
    ...
    const mapPanel=$('.map-panel');if(mapPanel){const peak=Math.max(d.rival||0,d.police||0,d.unrest||0);mapPanel.dataset.mapAlert=peak>=72?'critical':peak>=48?'watch':'calm';mapPanel.dataset.mapOwner=d.owner||'neutral';mapPanel.dataset.mapEvent=event?'active':'none';}const call=$('#mapCallout');if(call)call.innerHTML=`<strong>${esc(l.title)} · ${esc(engine.ownerLabel(d.owner))}</strong><span>${esc(l.kind)} · Kontrolle ${d.control}% · Rivalen ${d.rival}% · Polizei ${d.police}% · Unruhe ${d.unrest}% · Intel ${d.intel}/5${event?` · ${esc(event.title)}`:''}</span>`;
