(function(root,factory){
  ...
  class RevivalGameEngine extends BaseGameEngine{
    ...
    endTurn(){if(this.state.combatSession)return{ok:false,reason:'Laufenden Kampf zuerst beenden.'};if(this.state.casino?.poker?.status==='playing')return{ok:false,reason:'Offene Pokerrunde zuerst abschließen.'};const before=this.state.turn;this.log('turn','Zug bewusst beendet. Die Stadt arbeitet weiter.');this.advanceTurn();return{ok:true,fromTurn:before,turn:this.state.turn};}
