...
func _init(p_target_location_id: String, p_transaction_id: String, p_actor_id: String = "player") -> void:
...
func validate(state: GameSessionState, context: Dictionary) -> GameResult:
	...
	if world_registry == null or world_registry.definition() == null:
		return GameResult.failure(GameError.new("WORLD-ERR-007", "Weltregistry fehlt im Ausführungskontext."))
	var definition := world_registry.definition()
	if definition.location(target_location_id).is_empty():
		return GameResult.failure(GameError.new("WORLD-ERR-008", "Zielort ist unbekannt.", {"location_id": target_location_id}))
	if target_location_id == state.current_location_id:
		return GameResult.failure(GameError.new("WORLD-ERR-009", "Spieler befindet sich bereits am Zielort."))
	var connection := definition.connection(state.current_location_id, target_location_id)
	if connection.is_empty():
		return GameResult.failure(GameError.new("WORLD-ERR-010", "Zwischen den Orten existiert keine direkte Verbindung.", {"from": state.current_location_id, "to": target_location_id}))
	for requirement_value in connection.get("requirements", []):
		...
		if str(requirement.get("type", "")) == "flag_equals":
			var actual: Variant = state.flags.get(str(requirement.get("flag_id", "")))
			if actual != requirement.get("value"):
				return GameResult.failure(GameError.new("WORLD-ERR-014", "Reisevoraussetzung ist nicht erfüllt.", {"requirement": requirement}))
	for cost_value in connection.get("costs", []):
		...
		if state.resource_amount(resource_id) < amount:
			return GameResult.failure(GameError.new("WORLD-ERR-015", "Ressourcen reichen für die Reise nicht aus.", {"resource_id": resource_id, "required": amount}))
...
func build_effects(state: GameSessionState, context: Dictionary) -> GameResult:
	...
	var connection := world_registry.definition().connection(state.current_location_id, target_location_id)
	var effects: Array = []
	for cost_value in connection.get("costs", []):
		var cost: Dictionary = cost_value
		effects.append(ModifyResourceEffect.new(str(cost.get("resource_id", "")), -int(cost.get("amount", 0)), "world_travel"))
	effects.append(TravelToLocationEffect.new(str(connection.get("id", "")), target_location_id, int(connection.get("duration_steps", 1))))
	var mission_registry: MissionRegistry = context.get("mission_registry") as MissionRegistry
	if mission_registry != null:
		var signal_command := ApplyMissionSignalCommand.new("world.location_entered", {"location_id": target_location_id}, "%s:mission-signal" % transaction_id, actor_id)
