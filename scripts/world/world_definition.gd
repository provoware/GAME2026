...
static func from_dict(data: Dictionary) -> GameResult:
	...
	for location_value in data.get("locations", []):
		...
		if location_id.is_empty() or definition.locations.has(location_id):
			return GameResult.failure(GameError.new("WORLD-ERR-001", "Orts-ID fehlt oder ist doppelt.", {"location_id": location_id}))
	...
	if definition.id.is_empty() or not definition.locations.has(definition.start_location_id):
		return GameResult.failure(GameError.new("WORLD-ERR-002", "Weltdefinition besitzt keinen gültigen Startort."))
	for connection_value in definition.connections:
		var connection: Dictionary = connection_value
		if not definition.locations.has(str(connection.get("from_location_id", ""))) or not definition.locations.has(str(connection.get("to_location_id", ""))):
			return GameResult.failure(GameError.new("WORLD-ERR-003", "Verbindung verweist auf unbekannten Ort.", {"connection": connection}))
...
func outgoing(location_id: String) -> Array:
	...
	for value in connections:
		...
		var source := str(candidate.get("from_location_id", ""))
		var target := str(candidate.get("to_location_id", ""))
		if source == location_id or (bool(candidate.get("bidirectional", false)) and target == location_id):
